...
use crate::types::{AnalysisResult, ScoreComponents, RepositoryInfo, CentralityScores};
...
pub trait HeuristicScorer: Send + Sync {
    ...
    fn score_file(&self, file_info: &FileInfo, repo_info: &RepositoryInfo) -> Result<ScoreComponents>;
    
    /// Batch score multiple files (can be optimized for cross-file analysis)
    fn score_batch(&self, files: &[&FileInfo], repo_info: &RepositoryInfo) -> Result<Vec<ScoreComponents>> {
        files.iter()
...
pub trait RepositoryAnalyzer: Send + Sync {
    ...
    async fn analyze_repository(&self, root_path: &Path, config: &Config) -> Result<RepositoryInfo>;
...
pub trait GitIntegration: Send + Sync {
    ...
    async fn analyze_churn(&self, root_path: &Path, depth: usize) -> Result<Vec<crate::types::ChurnInfo>>;
...
impl DependencyGraph {
    ...
    pub fn stats(&self) -> crate::types::GraphStats {
        ...
        let in_degree_avg = if total_nodes > 0 { in_degree_sum as f64 / total_nodes as f64 } else { 0.0 };
        let out_degree_avg = if total_nodes > 0 { out_degree_sum as f64 / total_nodes as f64 } else { 0.0 };
        
        let in_degree_max = self.reverse_edges.iter().map(|adj| adj.len()).max().unwrap_or(0);
        let out_degree_max = self.edges.iter().map(|adj| adj.len()).max().unwrap_or(0);
        
        let possible_edges = if total_nodes > 1 { total_nodes * (total_nodes - 1) } else { 0 };
        let graph_density = if possible_edges > 0 { total_edges as f64 / possible_edges as f64 } else { 0.0 };
...
pub trait LanguageExtension: Send + Sync {
    ...
    async fn extract_dependencies(&self, content: &str, language: crate::file::Language) -> Result<Vec<String>>;
    
    /// Detect if file is an entrypoint (main, index, etc.)
    async fn is_entrypoint(&self, file_info: &FileInfo) -> Result<bool>;
    
    /// Extract documentation/comments
    async fn extract_documentation(&self, content: &str, language: crate::file::Language) -> Result<Vec<DocumentationBlock>>;
