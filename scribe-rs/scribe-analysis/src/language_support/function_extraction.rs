...
impl FunctionExtractor {
    ...
    pub fn new(language: AstLanguage) -> Result<Self> {
        ...
        let (function_query, class_query) = if let Some(ts_language) = language.tree_sitter_language() {
    ...
    fn create_function_query(language: AstLanguage, ts_language: Language) -> Result<Option<Query>> {
        let query_string = match language {
            AstLanguage::Python => r#"
                (function_definition) @function.definition
            "#,
            AstLanguage::JavaScript | AstLanguage::TypeScript => r#"
            ...
            AstLanguage::Java => r#"
                (method_declaration) @function.definition
            "#,
            AstLanguage::C | AstLanguage::Cpp => r#"
                (function_definition) @function.definition
            "#,
            AstLanguage::Ruby => r#"
                (method) @function.definition
            "#,
            AstLanguage::CSharp => r#"
                (method_declaration) @function.definition
            "#,
            _ => return Ok(None),
        };
    ...
    pub fn extract_functions(&mut self, content: &str) -> Result<Vec<FunctionInfo>> {
        ...
        if let Some(query) = &self.function_query {
            ...
            for query_match in matches {
                if let Some(function_info) = self.extract_function_from_match(&query_match, content, &tree)? {
    ...
    pub fn extract_classes(&mut self, content: &str) -> Result<Vec<ClassInfo>> {
        ...
        if let Some(query) = &self.class_query {
            ...
            for query_match in matches {
                if let Some(class_info) = self.extract_class_from_match(&query_match, content, &tree)? {
            ...
            match child.kind() {
                "argument_list" | "superclass" | "inheritance" => {
                    ...
                    loop {
                        ...
                        if parent_node.kind() == "identifier" || parent_node.kind() == "type_identifier" {
