...
use tree_sitter::{Parser, Language as TsLanguage, Node, Tree};
...
impl TemplateEngine {
    ...
    pub fn score_boost(&self) -> f64 {
        match self {
            ...
            TemplateEngine::Handlebars | TemplateEngine::Mustache | 
            TemplateEngine::Jinja2 | TemplateEngine::Twig | 
            TemplateEngine::Liquid => 1.5,
            
            // Moderate priority for framework templates
            TemplateEngine::Vue | TemplateEngine::Svelte | 
            TemplateEngine::React => 1.3,
            
            // Standard boost for other template engines
            TemplateEngine::Ejs | TemplateEngine::Pug | TemplateEngine::Erb |
...
impl std::fmt::Debug for TemplateDetector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TemplateDetector")
            ...
            .field("content_cache", &format!("[{} cached items]", self.content_cache.len()))
...
impl TemplateDetector {
    ...
    pub fn new() -> Result<Self> {
        ...
        html_parser.set_language(tree_sitter_html::language())
            .map_err(|e| scribe_core::ScribeError::parse(format!("Failed to set HTML language: {}", e)))?;
    ...
    fn is_in_template_directory(&self, path: &Path) -> bool {
        ...
        TEMPLATE_DIRECTORIES.iter().any(|dir| path_str.contains(dir))
    }
    ...
    fn detect_by_content_patterns(&mut self, path: &Path) -> Result<Option<TemplateDetectionResult>> {
        ...
        for pattern in TEMPLATE_PATTERNS.iter() {
            let occurrences = self.count_pattern_occurrences(&content, &pattern.open_tag, &pattern.close_tag);
    ...
    fn read_file_content(&mut self, path: &Path) -> Result<String> {
        ...
        if self.content_cache.len() < self.max_cache_size {
            self.content_cache.insert(path.to_path_buf(), content.clone());
        }
    ...
    fn analyze_with_ast(&mut self, path: &Path, content: &str) -> Result<Option<TemplateDetectionResult>> {
    ...
    fn node_contains_patterns(&self, node: &Node, patterns: &[&str]) -> bool {
        ...
        if patterns.iter().any(|&pattern| node.kind().contains(pattern)) {
            return true;
        }
