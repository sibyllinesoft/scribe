...
use super::types::{ScoreComponents, HeuristicWeights};
...
pub fn calculate_final_score(
    ...
    let weighted_sum = 
        weights.doc_weight * normalized_scores.doc_score +
...
impl ScoreComponents {
    ...
    pub fn top_factors(&self, count: usize) -> Vec<(String, f64)> {
        let mut factors = vec![
            ("doc_score".to_string(), self.doc_score * self.weights.doc_weight),
            ("readme_score".to_string(), self.readme_score * self.weights.readme_weight),
            ("import_score".to_string(), self.import_score * self.weights.import_weight),
            ("path_score".to_string(), self.path_score * self.weights.path_weight),
            ("test_link_score".to_string(), self.test_link_score * self.weights.test_link_weight),
            ("churn_score".to_string(), self.churn_score * self.weights.churn_weight),
            ("centrality_score".to_string(), self.centrality_score * self.weights.centrality_weight),
            ("entrypoint_score".to_string(), self.entrypoint_score * self.weights.entrypoint_weight),
            ("examples_score".to_string(), self.examples_score * self.weights.examples_weight),
    ...
    pub fn is_high_confidence(&self) -> bool {
        let contributing_factors = self.top_factors(11)
            .iter()
