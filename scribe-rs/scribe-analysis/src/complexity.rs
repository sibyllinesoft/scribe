...
impl ComplexityAnalyzer {
    ...
    pub fn analyze_content(&self, content: &str, language: &str) -> Result<ComplexityMetrics> {
        ...
        let maintainability_index = if self.config.enable_maintainability_index {
            self.calculate_maintainability_index(&line_metrics, cyclomatic_complexity, function_count)
        } else {
            0.0
        };
    ...
    fn get_complexity_keywords(&self, language: &str) -> Vec<&'static str> {
        match language.to_lowercase().as_str() {
            ...
            "javascript" | "typescript" => vec![
                "if", "else if", "while", "for", "catch", "case", 
                "&&", "||", "?", "break", "continue", "return"
            ],
            "java" | "c#" => vec![
                "if", "else if", "while", "for", "foreach", "catch", 
    ...
    fn get_function_keywords(&self, language: &str) -> Vec<&'static str> {
        match language.to_lowercase().as_str() {
            ...
            "javascript" | "typescript" => vec!["function ", "const ", "let ", "var ", "async function "],
    ...
    fn has_recursive_call(&self, line: &str, _language: &str) -> bool {
        ...
        line.contains("recursive") || line.contains("recurse")
    }
    
    /// Calculate maintainability index (Halstead-based)
    fn calculate_maintainability_index(&self, line_metrics: &LineMetrics, cyclomatic: usize, functions: usize) -> f64 {
    ...
    fn analyze_language_specific(&self, content: &str, language: &str) -> LanguageSpecificMetrics {
        ...
        match language.to_lowercase().as_str() {
            "rust" => {
                complexity_factors.insert("ownership_complexity".to_string(), self.calculate_ownership_complexity(content));
                complexity_factors.insert("trait_complexity".to_string(), self.count_trait_usage(content) as f64);
                complexity_factors.insert("macro_complexity".to_string(), self.count_macro_usage(content) as f64);
            },
            "python" => {
                complexity_factors.insert("decorator_complexity".to_string(), self.count_decorators(content) as f64);
                complexity_factors.insert("comprehension_complexity".to_string(), self.count_comprehensions(content) as f64);
            },
            "javascript" | "typescript" => {
                complexity_factors.insert("closure_complexity".to_string(), self.count_closures(content) as f64);
                complexity_factors.insert("promise_complexity".to_string(), self.count_async_patterns(content) as f64);
            },
            _ => {
    ...
    fn count_imports(&self, content: &str, language: &str) -> usize {
        ...
        content.lines()
            .filter(|line| {
                let trimmed = line.trim();
                import_patterns.iter().any(|pattern| trimmed.starts_with(pattern))
            })
            .count()
    }
    ...
    fn count_exports(&self, content: &str, language: &str) -> usize {
        ...
        content.lines()
            .filter(|line| {
                let trimmed = line.trim();
                export_patterns.iter().any(|pattern| trimmed.contains(pattern))
            })
            .count()
    }
    ...
    fn count_async_patterns(&self, content: &str) -> usize {
        content.lines()
            .filter(|line| {
                ...
                line.contains("Promise") || line.contains(".then(")
            })
            .count()
    }
}
...
impl ComplexityMetrics {
    ...
    pub fn complexity_score(&self) -> f64 {
        ...
        (cyclomatic_score * 0.3 + 
         nesting_score * 0.2 + 
         cognitive_score * 0.3 + 
         maintainability_score * 0.2).min(1.0)
    }
    ...
    pub fn exceeds_thresholds(&self, thresholds: &ComplexityThresholds) -> Vec<String> {
        ...
        if self.cyclomatic_complexity > thresholds.cyclomatic_warning {
            warnings.push(format!("High cyclomatic complexity: {}", self.cyclomatic_complexity));
        }
        ...
        if self.average_function_length > thresholds.function_length_warning as f64 {
            warnings.push(format!("Long functions: avg {:.1} lines", self.average_function_length));
        }
        
        if self.maintainability_index < thresholds.maintainability_warning {
            warnings.push(format!("Low maintainability: {:.1}", self.maintainability_index));
        }
