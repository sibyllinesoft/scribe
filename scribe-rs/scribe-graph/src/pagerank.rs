...
impl PageRankComputer {
    ...
    pub fn compute(&self, graph: &DependencyGraph) -> Result<PageRankResults> {
        ...
        let convergence_rate = if convergence_history.len() > 1 {
            ...
            if first > 0.0 { (first - last) / first } else { 0.0 }
        } else {
            0.0
        };
    ...
    fn compute_iteration_parallel(
        ...
        let new_scores: Vec<(NodeId, f64)> = nodes.par_iter()
            .map(|node| {
                let mut new_score = teleport_prob;
    ...
    fn estimate_memory_usage(&self, num_nodes: usize) -> f64 {
        ...
        let score_map_size = num_nodes * (std::mem::size_of::<String>() + std::mem::size_of::<f64>());
...
impl PageRankResults {
    ...
    pub fn top_nodes(&self, k: usize) -> Vec<(NodeId, f64)> {
        let mut sorted_scores: Vec<_> = self.scores.iter()
...
mod tests {
    ...
    fn test_pagerank_computation() {
        ...
        println!("Total score: {}, Number of nodes: {}", total_score, results.scores.len());
    ...
    fn test_pagerank_parallel_vs_sequential() {
        ...
        for node in graph.nodes() {
            ...
            assert!(diff < 1e-6, "Scores differ too much for node {}: seq={:.8}, par={:.8}", 
                    node, seq_score, par_score);
        }
    ...
    fn test_configuration_variants() {
        ...
        for config in configs {
            ...
            println!("Config {:?}: converged in {} iterations", 
