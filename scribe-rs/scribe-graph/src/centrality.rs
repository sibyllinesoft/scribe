...
use crate::pagerank::{PageRankComputer, PageRankResults, PageRankConfig};
use crate::statistics::{GraphStatisticsAnalyzer, GraphAnalysisResults};
...
impl CentralityCalculator {
    ...
    pub fn integrate_with_heuristics(
        ...
        let normalized_centrality = self.normalize_centrality_scores(
            &centrality_results.pagerank_scores,
            heuristic_scores,
        )?;
        ...
        for (file_path, heuristic_score) in heuristic_scores {
            ...
            let integrated_score = heuristic_weight * heuristic_score + 
                                 centrality_weight * boosted_centrality;
    ...
    /// Build dependency graph from scan results
    fn build_dependency_graph<T>(&self, scan_results: &[T]) -> Result<(DependencyGraph, ImportDetectionStats)> 
    where 
        T: ScanResult + Sync,
    {
        ...
        T: ScanResult,
    {
        ...
        let file_path_map: HashMap<&str, &T> = scan_results.iter()
        ...
        T: ScanResult + Sync,
    {
        ...
        let total_imports: usize = scan_results.iter()
    ...
    fn normalize_centrality_scores(
        ...
        match self.config.integration.normalization_method {
            NormalizationMethod::MinMax => self.normalize_min_max(centrality_scores, heuristic_scores),
    ...
    fn normalize_min_max(
        ...
        let min_centrality = centrality_values.iter().fold(f64::INFINITY, |a, &b| a.min(b));
        let max_centrality = centrality_values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        ...
        let max_heuristic = if heuristic_values.is_empty() {
            ...
            heuristic_values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b))
        };
        ...
        if (max_centrality - min_centrality).abs() < f64::EPSILON {
            ...
            for (path, &score) in centrality_scores {
                let normalized_score = ((score - min_centrality) / (max_centrality - min_centrality)) * max_heuristic;
    ...
    /// Z-score normalization
    fn normalize_z_score(&self, centrality_scores: &HashMap<String, f64>) -> Result<HashMap<String, f64>> {
    ...
    /// Rank-based normalization
    fn normalize_rank(&self, centrality_scores: &HashMap<String, f64>) -> Result<HashMap<String, f64>> {
        let mut scored_files: Vec<_> = centrality_scores.iter()
...
impl ImportDetector {
        ...
        T: ScanResult,
    {
        ...
        match language.as_deref() {
            ...
            Some("javascript") | Some("typescript") => self.resolve_js_import(import_str, current_path, file_map),
        ...
        T: ScanResult,
    {
        ...
        if let Some(parent) = current_path.parent() {
            ...
            let relative_candidates: Vec<String> = candidates.iter()
    ...
    fn is_python_stdlib(&self, import_str: &str) -> bool {
        let stdlib_modules = [
            ...
            "typing", "datetime", "math", "random", "string", "pathlib", "io",
            "csv", "xml", "html", "urllib", "http", "email", "logging", "unittest",
...
impl CentralityResults {
    ...
    pub fn top_files_by_centrality(&self, k: usize) -> Vec<(String, f64)> {
        let mut scored_files: Vec<_> = self.pagerank_scores.iter()
            .map(|(path, &score)| (path.clone(), score))
            .collect();
    ...
    pub fn summary(&self) -> String {
        format!(
            ...
            self.import_stats.language_breakdown.iter()
...
mod tests {
    ...
    fn test_import_detection() {
        ...
        assert_eq!(detector.detect_language("main.py"), Some("python".to_string()));
        assert_eq!(detector.detect_language("app.js"), Some("javascript".to_string()));
    ...
    fn test_centrality_calculation() {
        ...
        assert_eq!(results.integration_metadata.files_with_centrality, results.pagerank_scores.len());
    ...
    fn test_heuristics_integration() {
        ...
        for (file, integrated_score) in &integrated_scores {
            ...
            println!("File {}: heuristic={:.3}, integrated={:.3}", 
    ...
    fn test_normalization_methods() {
        ...
        let normalized = calculator.normalize_min_max(&centrality_scores, &heuristic_scores).unwrap();
    ...
    fn test_import_resolution() {
        ...
        let file_map: HashMap<&str, &MockScanResult> = scan_results.iter()
            .map(|result| (result.path(), result))
            .collect();
        ...
        let resolved_config = detector.resolve_import(
            "src.config",
            "src/main.py",
            &file_map
        );
