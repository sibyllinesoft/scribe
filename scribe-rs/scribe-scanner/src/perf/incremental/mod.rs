...
impl IncrementalScanner {
    ...
    pub(crate) async fn discover_files(&self) -> Result<Vec<PathBuf>> {
    ...
    pub(crate) async fn detect_file_change(
        &self,
        entry: &ManifestEntry,
        metadata: &Metadata,
        file_path: &Path,
    ...
    pub(crate) async fn calculate_file_hash(&self, file_path: &Path) -> Result<u64> {
        let metadata = fs::metadata(file_path).await?;
    ...
    pub(crate) fn get_inode(&self, metadata: &Metadata) -> u64 {
        #[cfg(unix)]
        {
    ...
    pub(crate) fn get_device(&self, metadata: &Metadata) -> u64 {
        #[cfg(unix)]
        {
            use std::os::unix::fs::MetadataExt;
            metadata.dev()
