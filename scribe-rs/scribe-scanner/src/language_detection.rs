...
impl LanguageDetector {
    ...
    pub fn detect_language(&self, path: &Path) -> Language {
        match self.strategy {
            DetectionStrategy::ExtensionOnly => self.detect_by_extension(path),
            _ => {
    ...
    fn initialize_filename_patterns(&mut self) {
        ...
        for (filename, language) in patterns {
            self.filename_patterns
                .insert(filename.to_string(), language);
        }
    }
    ...
    fn initialize_content_signatures(&mut self) {
        ...
        if let Ok(compiled_patterns) = self.compile_patterns(python_patterns) {
            ...
            self.content_signatures
                .insert(Language::Python, python_sigs);
        }
        ...
        if let Ok(compiled_patterns) = self.compile_patterns(js_patterns) {
            ...
            self.content_signatures
                .insert(Language::JavaScript, js_sigs);
        }
    ...
    fn initialize_syntax_analyzers(&mut self) {
        ...
        self.syntax_analyzers
            .insert(Language::Python, python_analyzer);
        ...
        self.syntax_analyzers
            .insert(Language::JavaScript, js_analyzer);
    ...
    fn analyze_content_signatures_optimized(
        &self,
        content: &str,
                ...
                if let Some(parser) = self.ast_parsers.get_mut(&language) {
                    if let Some(tree) = parser.parse(content, None) {
                        ...
                        let structural_score =
                            self.calculate_structural_score(&root_node, analyzer);
    ...
    fn aggregate_detection_results(
        &self,
    ...
    fn apply_dominant_language_bias(
        &self,
        mut result: DetectionResult,
