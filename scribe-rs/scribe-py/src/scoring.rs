...
use scribe_analysis::{
    CodeAnalyzer, 
    heuristics::{HeuristicScorer as AnalysisScorer, ScoreComponents, HeuristicWeights, ScanResult}
};
...
impl PyHeuristicScorer {
    ...
    pub fn score_file<'py>(
        ...
        future_into_py(py, async move {
            ...
            // Perform scoring
            let scores = engine_guard.score_file(&file_info, &weights_guard)
                .await
                .map_err(rust_error_to_py_err)?;
            
            // Convert to Python dict
            Python::with_gil(|py| {
                score_components_to_py_dict(py, &scores)
            })
        })
    }
    ...
    pub fn get_top_files(
        ...
        Python::with_gil(|py| {
            ...
            for (path, score) in top_files {
                let tuple = pyo3::types::PyTuple::new(py, &[path.to_object(py), score.to_object(py)]);
    ...
    pub fn get_weights(&self) -> PyResult<PyObject> {
        ...
        Python::with_gil(|py| {
            heuristic_weights_to_py_dict(py, &weights_guard)
        })
    }
...
pub fn get_default_weights() -> PyResult<PyObject> {
    ...
    Python::with_gil(|py| {
        heuristic_weights_to_py_dict(py, &weights)
    })
}
...
mod tests {
    ...
    fn test_scorer_creation() {
        Python::with_gil(|py| {
            let scorer = HeuristicScorer::new(None, None);
            assert!(scorer.is_ok());
            ...
            let scorer_with_weights = HeuristicScorer::new(None, Some(weights_dict));
            assert!(scorer_with_weights.is_ok());
        });
    }

    #[test]
    fn test_get_default_weights() {
        Python::with_gil(|py| {
            let weights = get_default_weights();
            assert!(weights.is_ok());
            
            let weights_obj = weights.unwrap();
            let weights_dict: &PyDict = weights_obj.downcast(py).unwrap();
            
            // Check that required weight fields are present
            assert!(weights_dict.contains("documentation").unwrap());
            assert!(weights_dict.contains("complexity").unwrap());
            assert!(weights_dict.contains("functions").unwrap());
        });
    }

    #[test]
    fn test_create_default_scorer() {
