...
use scribe_core::{CentralityScores, Config, FileInfo, GraphStats};
use scribe_graph::{DependencyGraph, GraphStatisticsAnalyzer, PageRankComputer};
...
impl PageRankAnalyzer {
    ...
    pub fn new(
        ...
        let engine = PageRankEngine::with_config(engine_config).map_err(rust_error_to_py_err)?;

        let analyzer = GraphAnalyzer::new(analyzer_config.clone()).map_err(rust_error_to_py_err)?;
    ...
    pub fn analyze_dependencies<'py>(
        ...
        future_into_py(py, async move {
            ...
            for (idx, path) in file_paths.iter().enumerate() {
                ...
                let dependencies = analyzer_guard
                    .analyze_file_dependencies(&file_info)
                    .await
        ...
        future_into_py(py, async move {
            ...
            for measure in requested_measures {
                let centrality_scores = match measure.as_str() {
                    "pagerank" => analyzer_guard
                        .calculate_pagerank_centrality(&dep_graph)
                        .await
                        .map_err(rust_error_to_py_err)?,
                    "betweenness" => analyzer_guard
                        .calculate_betweenness_centrality(&dep_graph)
                        .await
                        .map_err(rust_error_to_py_err)?,
                    "closeness" => analyzer_guard
                        .calculate_closeness_centrality(&dep_graph)
                        .await
                        .map_err(rust_error_to_py_err)?,
                    "degree" => analyzer_guard
                        .calculate_degree_centrality(&dep_graph)
                        .await
                        .map_err(rust_error_to_py_err)?,
                    "eigenvector" => analyzer_guard
                        .calculate_eigenvector_centrality(&dep_graph)
                        .await
                        .map_err(rust_error_to_py_err)?,
                    _ => {
                        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                            "Unknown centrality measure: {}",
                            measure
                        )));
                    }
                };
            ...
            Python::with_gil(|py| {
                ...
                let all_nodes: Vec<String> = dep_graph.get_all_nodes().into_iter().collect();
    ...
    pub fn get_graph_statistics<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            if let Some(dep_graph) = graph_guard.as_ref() {
                let analyzer_guard = analyzer.read().await;
                let stats = analyzer_guard
                    .calculate_graph_statistics(dep_graph)
                    .await
                    .map_err(rust_error_to_py_err)?;
        ...
        future_into_py(py, async move {
            ...
            if let Some(dep_graph) = graph_guard.as_ref() {
                match export_format.as_str() {
                    "graphml" => {
                        dep_graph
                            .export_graphml(&output, include_meta)
                            .await
                            .map_err(rust_error_to_py_err)?;
                    }
                    "dot" => {
                        dep_graph
                            .export_dot(&output, include_meta)
                            .await
                            .map_err(rust_error_to_py_err)?;
                    }
                    "json" => {
                        dep_graph
                            .export_json(&output, include_meta)
                            .await
                            .map_err(rust_error_to_py_err)?;
                    }
                    "csv" => {
    ...
    pub fn find_strongly_connected_components<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            if let Some(dep_graph) = graph_guard.as_ref() {
                let analyzer_guard = analyzer.read().await;
                let components = analyzer_guard
                    .find_strongly_connected_components(dep_graph)
                    .await
                    .map_err(rust_error_to_py_err)?;
    ...
    pub fn find_circular_dependencies<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            if let Some(dep_graph) = graph_guard.as_ref() {
                let analyzer_guard = analyzer.read().await;
                let cycles = analyzer_guard
                    .find_circular_dependencies(dep_graph)
                    .await
                    .map_err(rust_error_to_py_err)?;
...
mod tests {
    ...
    fn test_pagerank_analyzer_creation() {
        Python::with_gil(|py| {
            ...
            let custom_analyzer = PageRankAnalyzer::new(Some(0.9), Some(200), Some(1e-8), None);
