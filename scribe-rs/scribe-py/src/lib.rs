...
pub use error::{
    rust_error_to_py_err, rust_result_to_py, AnalysisException, ConfigurationException,
    PatternException, ScribeException, ToPyResult,
};

pub use config::PyConfig as Config;

pub use repository::{create_repository, find_repository_root, is_valid_repository, Repository};

pub use scoring::{
    create_default_scorer, create_scorer_with_weights, get_default_weights, HeuristicScorer,
};

pub use graph::{create_pagerank_analyzer, create_pagerank_analyzer_with_config, PageRankAnalyzer};

pub use patterns::{
    create_pattern_matcher, get_supported_languages, validate_pattern, PatternMatcher,
};
...
fn _scribe_py(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add(
        ...
        _py.get_type::<ConfigurationException>(),
    )?;
    ...
    m.add(
        "__doc__",
        "High-performance code analysis library with Rust backend",
    )?;
...
fn analyze_repository_comprehensive<'py>(
    ...
    future_into_py(py, async move {
        ...
        let analyzer = PageRankAnalyzer::new(pagerank_damping, None, None, None)?;

        // Analyze dependencies
        let centrality_scores = analyzer
        ...
        let final_results = Python::with_gil(|py| {
            ...
            scorer.combine_with_centrality(scored_dict, centrality_dict, None)
        })?;
