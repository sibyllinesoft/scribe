...
use scribe_core::{Config, FileInfo, RepositoryInfo, AnalysisResult};
...
impl Repository {
    ...
    pub fn get_repository_info<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            let analyzer = Analyzer::new(config_guard.clone())
                .map_err(rust_error_to_py_err)?;
            
            let info = analyzer.analyze_repository(&path)
                .await
                .map_err(rust_error_to_py_err)?;
        ...
        future_into_py(py, async move {
            ...
            let scanner_instance = {
                ...
                if scanner_guard.is_none() {
                    let config_guard = config.read().await;
                    let new_scanner = Scanner::new(config_guard.clone())
                        .map_err(rust_error_to_py_err)?;
    ...
    pub fn get_files_by_language<'py>(
        ...
        future_into_py(py, async move {
            ...
            let target_lang = py_to_language(&pyo3::types::PyString::new(
                pyo3::Python::with_gil(|py| py), &lang_filter
            ))?;
    ...
    pub fn get_size_stats<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            stats.insert("total_files".to_string(), info.size_stats.total_files as i64);
            stats.insert("total_lines".to_string(), info.size_stats.total_lines as i64);
            stats.insert("total_bytes".to_string(), info.size_stats.total_bytes as i64);
            stats.insert("largest_file_size".to_string(), info.size_stats.largest_file_size as i64);
            stats.insert("average_file_size".to_string(), info.size_stats.average_file_size as i64);
            stats.insert("median_file_size".to_string(), info.size_stats.median_file_size as i64);
    ...
    pub fn get_config<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            config_dict.insert("max_files".to_string(), config_guard.filtering.max_files as i64);
            config_dict.insert("max_file_size".to_string(), config_guard.filtering.max_file_size as i64);
            config_dict.insert("follow_symlinks".to_string(), config_guard.filtering.follow_symlinks as i64);
    ...
    pub fn get_git_stats<'py>(&self, py: Python<'py>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            if let Some(git_stats) = &info.git_stats {
                ...
                stats.insert("first_commit_date".to_string(), 
                           git_stats.first_commit_date.map(|d| d.timestamp()).unwrap_or(0));
                stats.insert("last_commit_date".to_string(), 
                           git_stats.last_commit_date.map(|d| d.timestamp()).unwrap_or(0));
                stats.insert("tracked_files".to_string(), git_stats.tracked_files as i64);
                stats.insert("modified_files".to_string(), git_stats.modified_files as i64);
...
mod tests {
    ...
    fn test_invalid_repository_path() {
        Python::with_gil(|py| {
            let repo = Repository::new("/nonexistent/path", None);
            assert!(repo.is_err());
        });
    }
    ...
    fn test_is_valid_repository() {
        ...
        assert!(!is_valid_repository("/nonexistent/path"));
    }
}