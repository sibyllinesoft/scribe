...
use scribe_patterns::{PatternMatcher as ScribePatternMatcher, MatchResult, MatcherOptions};
...
impl PatternMatcher {
    ...
    pub fn load_rules_from_file<'py>(&self, py: Python<'py>, rules_path: &str) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            ...
            let loaded_rules = engine_guard.load_rules_from_file(&path)
                .await
    ...
    /// Returns:
    ///     Number of rules loaded
    pub fn load_rules_from_json<'py>(&self, py: Python<'py>, rules_json: &str) -> PyResult<&'py PyAny> {
        let engine = self.engine.clone();
        let rules = self.rules.clone();
        ...
        future_into_py(py, async move {
            ...
            let loaded_rules = engine_guard.load_rules_from_json(&json_str)
                .await
                .map_err(rust_error_to_py_err)?;
    ...
    pub fn add_rule(&self, rule_dict: &PyDict) -> PyResult<()> {
        ...
        let name: String = rule_dict.get_item("name")
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyKeyError, _>("Missing 'name' key"))?
            .unwrap().extract()?;
            
        let pattern: String = rule_dict.get_item("pattern")
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyKeyError, _>("Missing 'pattern' key"))?
            .unwrap().extract()?;
            
        let language_str: String = rule_dict.get_item("language")
        ...
        let language = py_to_language(&pyo3::types::PyString::new(
            pyo3::Python::with_gil(|py| py), &language_str
        ))?;
        
        let description = rule_dict.get_item("description")?.map(|v| v.extract::<String>()).transpose()?
            .unwrap_or_else(|| format!("Pattern rule: {}", name));
            
        let category = rule_dict.get_item("category")?.map(|v| v.extract::<String>()).transpose()?
            .unwrap_or_else(|| "custom".to_string());
            
        let severity = rule_dict.get_item("severity")?.map(|v| v.extract::<String>()).transpose()?
        ...
        futures::executor::block_on(async {
            ...
            engine_guard.add_rule(rule.clone())
                .await
                .map_err(rust_error_to_py_err)?;
        ...
        future_into_py(py, async move {
            ...
            let language_matcher = {
                ...
                if !matchers_guard.contains_key(&file_info.language) {
                    let matcher = scribe_patterns::PatternMatcher::for_language(file_info.language.clone())
                        .map_err(rust_error_to_py_err)?;
            ...
            let matches = if filter.is_empty() {
                engine_guard.find_all_matches(&file_info, &language_matcher)
                    .await
                    .map_err(rust_error_to_py_err)?
            } else {
                engine_guard.find_matches_with_filter(&file_info, &language_matcher, &filter)
                    .await
                    .map_err(rust_error_to_py_err)?
            };
            ...
            Python::with_gil(|py| {
                ...
                for match_result in matches {
                    ...
                    match_dict.set_item("start_position", position_to_py(py, &match_result.start_position)?)?;
                    match_dict.set_item("end_position", position_to_py(py, &match_result.end_position)?)?;
            ...
            for (batch_idx, batch) in file_paths.chunks(batch_size).enumerate() {
                for path in batch {
                    ...
                    let language_matcher = {
                        ...
                        if !matchers_guard.contains_key(&file_info.language) {
                            let matcher = match scribe_patterns::PatternMatcher::for_language(file_info.language.clone()) {
                                Ok(matcher) => matcher,
                    ...
                    let matches = if filter.is_empty() {
                        match engine_guard.find_all_matches(&file_info, &language_matcher).await {
                            Ok(matches) => matches,
                            Err(_) => continue, // Skip files with match errors
                        }
                    } else {
                        match engine_guard.find_matches_with_filter(&file_info, &language_matcher, &filter).await {
                            Ok(matches) => matches,
                            Err(_) => continue, // Skip files with match errors
                        }
                    };
                ...
                if let Some(ref callback) = progress_callback {
                    ...
                    let current = if current > total_files { total_files } else { current };
    ...
    pub fn get_rules(
        ...
        Python::with_gil(|py| {
            ...
            let lang_filter = language_filter.map(|s| py_to_language(
    ...
    /// Returns:
    ///     Success status
    pub fn export_rules<'py>(&self, py: Python<'py>, output_path: &str, format: &str) -> PyResult<&'py PyAny> {
        let engine = self.engine.clone();
        ...
        future_into_py(py, async move {
            ...
            match export_format.as_str() {
                "json" => {
                    engine_guard.export_rules_json(&path)
                        .await
                        .map_err(rust_error_to_py_err)?;
                },
                "yaml" => {
                    engine_guard.export_rules_yaml(&path)
                        .await
                        .map_err(rust_error_to_py_err)?;
                },
                "toml" => {
                    engine_guard.export_rules_toml(&path)
                        .await
                        .map_err(rust_error_to_py_err)?;
                },
                _ => {
...
mod tests {
    ...
    fn test_validate_pattern() {
        // Valid patterns
        assert_eq!(validate_pattern(r"\d+").unwrap(), true);
        assert_eq!(validate_pattern(r"[a-zA-Z]+").unwrap(), true);
        
        // Invalid patterns
        assert_eq!(validate_pattern(r"[").unwrap(), false);
        assert_eq!(validate_pattern(r"(?P<incomplete").unwrap(), false);
    }
    ...
    fn test_get_supported_languages() {
        Python::with_gil(|py| {
            ...
            let lang_list: &PyList = languages.unwrap().downcast(py).unwrap();
            assert!(lang_list.len() > 0);
        });
    }
    ...
    fn test_add_rule() {
        Python::with_gil(|py| {
            ...
            let result = matcher.add_rule(rule_dict);
            assert!(result.is_ok());
        });
    }
}