...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (scenario_name, query, budget) in test_scenarios {
        ...
        println!("   Positioning overhead: {:+?}", duration_optimized - duration_original);
        ...
        for (i, file) in result_optimized.files.iter().take(head_count).enumerate() {
            ...
            let importance = if filename.contains("main") || filename.contains("lib") || 
                              filename.contains("index") || filename.ends_with(".toml") {
                "🏗️ ENTRY"
            } else {
                "general"
            };
            
            println!("      {}. {} ({}, relevance: {}, importance: {})", 
        ...
        if total_files > head_count + 3 {
            println!("      ... {} files (utilities, configs, helpers) ...", 
                tail_start - head_count);
        }
        
        println!("   📍 Conceptual TAIL Section ({}% - Core Functionality):", 
        ...
        for (i, file) in tail_files.iter().enumerate() {
            let filename = file.path.file_name().unwrap_or_default().to_string_lossy();
            let importance = if filename.contains("main") || filename.contains("lib") || 
                              filename.contains("mod") || filename.contains("core") {
                "🏗️ CORE"
        ...
        if query.is_some() {
            let head_query_matches = result_optimized.files.iter().take(head_count)
                .filter(|f| {
                    let filename = f.path.file_name().unwrap_or_default().to_string_lossy().to_lowercase();
                    let path = f.path.to_string_lossy().to_lowercase();
                    query.as_ref().map_or(false, |q| filename.contains(q) || path.contains(q))
                })
                .count();
            
            println!("      Query-relevant files in HEAD: {}/{} ({:.0}%)", 
                head_query_matches, head_count, 
                100.0 * head_query_matches as f64 / head_count.max(1) as f64);
        }
        
        let tail_core_files = result_optimized.files.iter().skip(tail_start)
            .filter(|f| {
                let filename = f.path.file_name().unwrap_or_default().to_string_lossy().to_lowercase();
                filename.contains("main") || filename.contains("lib") || 
                filename.contains("mod") || filename.contains("core")
            })
            .count();
        
        println!("      Core architecture files in TAIL: {}/{} ({:.0}%)", 
            tail_core_files, total_files - tail_start,
