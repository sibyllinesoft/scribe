...
use crate::streaming::{StreamingConfig, FileMetadata};
use crate::caching::CacheConfig;
use crate::parallel::ParallelConfig;
use crate::adaptive::AdaptiveConfig;
use crate::signatures::SignatureConfig;
use crate::memory::MemoryConfig;
use crate::metrics::{ScalingMetrics, BenchmarkResult};
...
impl ScalingConfig {
    ...
    pub fn with_token_budget(token_budget: usize) -> Self {
        Self {
            ...
            enable_intelligent_selection: true,
            selection_algorithm: Some(match token_budget {
                0..=2000 => "v2_quotas",
...
impl ScalingEngine {
    ...
    pub async fn new(config: ScalingConfig) -> ScalingResult<Self> {
        info!("Initializing scaling engine with configuration: {:?}", config);
    ...
    pub async fn process_repository(&mut self, path: &Path) -> ScalingResult<ProcessingResult> {
        ...
        if !path.is_dir() {
            return Err(ScalingError::path("Repository path is not a directory", path));
        }
    ...
    async fn process_with_intelligent_selection(&self, path: &Path) -> ScalingResult<ProcessingResult> {
        ...
        info!("Intelligent selection completed: {} files selected, {:.1}% token utilization", 
...
mod tests {
    ...
    fn test_file_type_classification() {
        ...
        assert_eq!(classify_file_type(Path::new("config.json")), "Configuration");
    }
}