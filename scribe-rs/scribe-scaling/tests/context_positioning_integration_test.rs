...
use scribe_scaling::{
    ScalingSelector, ScalingSelectionConfig, ContextPositioningConfig,
    SelectionAlgorithm
};
use std::fs;
...
async fn test_complete_context_positioning_workflow() {
    ...
    let result = selector.select_and_process_with_query(
        repo_path, 
        Some("authentication middleware security")
    ).await.unwrap();
    ...
    assert!(auth_relevant, "HEAD should contain authentication-related files with high query relevance");
}
...
async fn test_positioning_vs_no_positioning_comparison() {
    ...
    let result_pos = selector_pos.select_and_process_with_query(
        repo_path, 
    ...
    assert_eq!(result_no_pos.selected_files.len(), result_pos.selected_files.len());
    ...
    assert!(head_percentage >= 0.1 && head_percentage <= 0.4, 
        "HEAD percentage should be reasonable: got {:.2}", head_percentage);
    
    // MIDDLE should be the largest section
    assert!(middle >= head && middle >= tail, "MIDDLE should be largest section");
}
...
async fn test_centrality_calculation_accuracy() {
    ...
    let lib_file = positioned.positioning.head_files.iter()
    ...
    let utils_file = positioned.positioning.head_files.iter()
        .chain(positioned.positioning.middle_files.iter()) 
        .chain(positioned.positioning.tail_files.iter())
    ...
    if let (Some(lib), Some(utils)) = (lib_file, utils_file) {
        ...
        assert!(lib.centrality.combined >= utils.centrality.combined * 0.8,
            "lib.rs centrality ({:.3}) should be >= 80% of utils.rs centrality ({:.3})",
            lib.centrality.combined, utils.centrality.combined);
...
async fn test_query_relevance_accuracy() {
    ...
    let positioned = result.positioned_selection.unwrap();
    let all_files: Vec<_> = positioned.positioning.head_files.iter()
        .chain(positioned.positioning.middle_files.iter())
        .chain(positioned.positioning.tail_files.iter())
        .collect();
    ...
    let auth_file = all_files.iter().find(|f| {
        ...
        path_str.contains("auth") || path_str.contains("middleware") || path_str.contains("handlers")
    });
    ...
    if let (Some(auth), Some(non_auth)) = (auth_file, non_auth_file) {
        ...
        assert!(auth.query_relevance >= non_auth.query_relevance,
        ...
        assert!(has_query_relevance, "At least one file should have query relevance for 'authentication'");
    }
}
...
async fn test_configuration_options() {
    ...
    if total >= 10 {
        assert!(head_pct > 0.20 || head >= 2, "HEAD percentage should be >= 20% or at least 2 files: got {:.2} ({} files)", head_pct, head);
    } else {
    ...
    if total >= 10 {
        assert!(tail_pct <= 0.25 || tail <= 2, "TAIL percentage should be <= 25% or at most 2 files: got {:.2} ({} files)", tail_pct, tail);
    } else {
...
fn create_realistic_repository(repo_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Auth module (should be relevant to authentication queries)
    fs::write(repo_path.join("src/auth/mod.rs"),
        "pub mod middleware;\npub mod handlers;\n")?;
        
    fs::write(repo_path.join("src/auth/middleware.rs"),
        "pub fn authenticate() {}\npub fn authorize() {}\n")?;
        
    fs::write(repo_path.join("src/auth/handlers.rs"), 
        "pub fn login() {}\npub fn logout() {}\n")?;
    
    // API module 
    fs::write(repo_path.join("src/api/mod.rs"),
        "pub mod routes;\npub mod handlers;\n")?;
        
    fs::write(repo_path.join("src/api/routes.rs"),
        "use crate::auth;\npub fn setup_routes() {}\n")?;
        
    fs::write(repo_path.join("src/api/handlers.rs"),
        "pub fn handle_request() {}\n")?;
    
    // Utils module (should have lower centrality)
    fs::write(repo_path.join("src/utils.rs"),
        "pub fn helper_function() {}\npub fn format_data() {}\n")?;
    
    // Test files (should have low centrality)
...
fn create_centrality_test_repository(repo_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    fs::create_dir_all(repo_path.join("src"))?;
    ...
    fs::write(repo_path.join("src/core.rs"),
