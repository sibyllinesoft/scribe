...
async fn test_toggle_file_endpoint() {
    ...
    assert!(included_files.iter().any(|f| f.as_str() == Some("src/lib.rs")));
}
...
async fn test_toggle_file_multiple_times() {
    ...
    assert!(included_files1.iter().any(|f| f.as_str() == Some("src/main.rs")));
    ...
    assert!(!included_files2.iter().any(|f| f.as_str() == Some("src/main.rs")));
}
...
async fn test_save_bundle_endpoint() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let bundle_path = temp_dir.path().join("test_bundle.md").to_string_lossy().to_string();
